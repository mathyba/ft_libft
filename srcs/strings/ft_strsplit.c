...
/*
** Allocates and returns an array of null-terminated strings containing
** substrings resulting of the split of str at character c.
** The array is also null-terminated.
** If no occurence of c is found in str, returns NULL.
*/
...
char		**ft_strsplit(char const *s, char c)
{
	...
	if (!s)
		return (NULL);
	nb_words = ft_nbwords((char *)s, c, 0);
	if (!(str = (char **)malloc(sizeof(char*) * (nb_words + 1))))
		return (NULL);
	str[nb_words] = NULL;
	return (!nb_words ? str : fill_tab(str, (char *)s, nb_words, c));
}
